...
function openCardModal(card) {
    ...
    createdSpan.textContent = `Created: ${new Date(card.created_at).toLocaleDateString()} - Card #${card.id}`;
...
function setupModalEvents() {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            if (settingsOvl.style.display === 'flex') {
...
async function submitComment() {
    ...
    try {
        ...
        // Clear the text input but keep the author
        textInput.value = '';
...
function setupHeaderEvents() {
    ...
    settingsBtn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.bg-option').forEach(btn => {
            ...
            const rgb = btn.dataset.bg;
            const hex = rgbToHex(`rgb(${hexToRgb(rgb)})`);
            if (hexToRgb(currentBg) === rgb) {
